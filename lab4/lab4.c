...
Player *create_player(char name[], char id[])
{
	...
	*/
	...
	struct Player *new_player = (Player *)malloc(sizeof(Player));
...
PlayerRecord *create_leaf_record(Player *p)
{
	...
	*/
	...
	struct PlayerRecord *new_player_record = (PlayerRecord *)malloc(sizeof(PlayerRecord));
	new_player_record->player = p;
	new_player_record->game_records[WINS] = 0;
	new_player_record->game_records[LOSSES] = 0;
	new_player_record->left_child = NULL;
	new_player_record->right_child = NULL;
	new_player_record->parent = NULL;
	...
	int p1_wins, int p2_wins)
{
	...
	*/
	...
	struct PlayerRecord *combined_player_record = (PlayerRecord *)malloc(sizeof(PlayerRecord));
	combined_player_record->parent = NULL;
	p1->parent = combined_player_record;
	p2->parent = combined_player_record;

	if (p1_wins > p2_wins)
	{
		combined_player_record->left_child = p2;
		combined_player_record->right_child = p1;
		combined_player_record->player = p1->player;
		combined_player_record->game_records[WINS] = p1_wins;
		combined_player_record->game_records[LOSSES] = p2_wins;
	}
	else
	{
		combined_player_record->left_child = p1;
		combined_player_record->right_child = p2;
		combined_player_record->player = p2->player;
		combined_player_record->game_records[WINS] = p2_wins;
		combined_player_record->game_records[LOSSES] = p1_wins;
	}
	return combined_player_record;
}

int get_player_rank_helper(char player_id[], PlayerRecord *root, int level)
{
	if (!root)
	{
		return 0;
	...
	if (strcmp(root->player->id, player_id) == 0)
	{
		return level;
	}

	int left_level = get_player_rank_helper(player_id, root->left_child, level + 1);
	int right_level = get_player_rank_helper(player_id, root->right_child, level + 1);
...
int get_player_rank(char player_id[], PlayerRecord *root)
{
	...
	*/
	return get_player_rank_helper(player_id, root, 1);
}
